...
namespace MVCWebApp_CRUD_session.Controllers
{
...
public IActionResult DoAdd(Course newCourse)
        ...
        public IActionResult AddToCart(int id)
        {
            ...
            if (course == null)
            {
                System.Diagnostics.Debug.WriteLine($"AddToCart: Course ID {id} not found");
                return NotFound();
            }
            System.Diagnostics.Debug.WriteLine($"AddToCart: Adding course ID {id}, Title: {course.Title}, Subject is {(course.Subject == null ? "null" : $"not null, Name: {course.Subject.Name}")}");
            SessionServices.AddToCart(HttpContext.Session, course);
            return RedirectToAction("List");
        }

        public IActionResult ViewCart()
        {
            System.Diagnostics.Debug.WriteLine($"ViewCart: Fetching cart, Session is {(HttpContext.Session == null ? "null" : "not null")}");
            var cart = HttpContext.Session != null ? SessionServices.GetCart(HttpContext.Session) : new List<Course>();
            System.Diagnostics.Debug.WriteLine($"ViewCart: Cart contains {cart.Count} courses");
            ViewBag.Cart = cart;
            return View();
        }
        ...
        {
            System.Diagnostics.Debug.WriteLine($"RemoveFromCart: Removing course ID {id}");
            SessionServices.RemoveFromCart(HttpContext.Session, id);
        ...
        {
            System.Diagnostics.Debug.WriteLine("ClearCart: Clearing cart");
            SessionServices.ClearCart(HttpContext.Session);
            return RedirectToAction("ViewCart");
        }
        ...
        {
            System.Diagnostics.Debug.WriteLine("ClearSession: Clearing all session data");
